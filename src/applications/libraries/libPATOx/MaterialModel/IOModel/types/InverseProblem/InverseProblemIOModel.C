...
compareList_(noIOModel::materialDict_.subDict("IO").template lookupOrDefault<List<Tuple2<Tuple2<word,scalar>,Tuple2<word,vector>> >>("compareList",List<Tuple2<Tuple2<word,scalar>,Tuple2<word,vector>> >(0))),
dakotaMethod_(noIOModel::materialDict_.subDict("IO").template lookupOrDefault<word>("dakotaMethod",word::null)),
dakotaResultsFileName_(noIOModel::materialDict_.subDict("IO").template lookupOrDefault<fileName>("dakotaResultsFileName",fileName("results/results").expand())),
indexSamplingCoord_(compareList_.size(),-1),
indexSamplingFields_(compareList_.size(),-1),
outputDeltaTime_(noIOModel::materialDict_.subDict("IO").template lookupOrDefault<scalar>("outputDeltaTime",-1)),
outputTime_(0),
...
inline void Foam::InverseProblemIOModel::dakotaInit()
{
  ...
  forAll(compareList_,compI) {
    ...
    forAll(noIOModel::probingDictNames_, dictI) {
      ...
      SampleFunction func_(mesh_, phaseName_, dictName_, noIOModel::probingDictNames_[dictI]);
    ...
    compareSampleFunctions_.append(new SampleFunction(mesh_, phaseName_, dictName_, noIOModel::probingDictNames_[funcLabel_]));
