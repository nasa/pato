...
const Foam::word Foam::simpleTimeControlModel::modelName="TimeControl";
...
)
{
  ...
  if (MaterialDict.isDict("TimeControl")) {
    ...
    TimeControlModelTypeName =
        word(TimeControlModelDict.lookupOrDefault<word>("TimeControlType","no"));
  ...
  return autoPtr<simpleTimeControlModel>(cstrIter()(mesh, regionName));
}
