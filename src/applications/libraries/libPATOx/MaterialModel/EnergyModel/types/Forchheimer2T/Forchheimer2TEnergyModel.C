...
Foam::Forchheimer2TEnergyModel::Forchheimer2TEnergyModel
(
    const fvMesh& mesh,
    const word& dictName
)
...
Hv_(createVolField<scalar>("Hv",dimensionedScalar("0",dimensionSet(1,-1,-3,-1,0,0,0),scalar(0)))),
...
Hv0_(createDimScalarProp("Hv0","yes",dimensionedScalar("Hv0",dimensionSet(1,-1,-3,-1,0,0,0),0))),
HvType_(createWordProp("HvType","yes","constant")),
...
GammaHg(createVolField<tensor>("GammaHg",dimensionedTensor("0", dimensionSet(0,2,-1,0,0,0,0), tensor::zero))),
...
void Foam::Forchheimer2TEnergyModel::update()
{
...
void Foam::Forchheimer2TEnergyModel::beforeSolve()
{
  ...
  GammaHg.field() *= h_g.ref()*p.ref()*M.ref()/(R.value()*Tg.ref());
...
void Foam::Forchheimer2TEnergyModel::afterSolve()
{
  if(this->debug_) {
    forAll(Ts, cellI) {
      if (Ts[cellI]<0) {
