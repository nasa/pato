...
Foam::gaussianBoundaryMappingModel::gaussianBoundaryMappingModel
(
    const fvMesh& mesh,
...
simpleBoundaryMappingModel(mesh, neededFields, dict)
{
  ...
  forAll(mappingFieldsName_, fieldI) {
    ...
    Info  << fieldCoeffs_[fieldI][0] << "*exp[-{(t-" << fieldCoeffs_[fieldI][1] << ")/" << fieldCoeffs_[fieldI][2] << "}^2]" << endl;
...
void Foam::gaussianBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)
{
  if(foundInList(fieldName,mappingFieldsName_ )) {
    ...
    if (mesh_.objectRegistry::foundObject<volScalarField>(mappingFieldsName_[fieldI])) {
      ...
      forAll(field_.boundaryField()[patchID], faceI) {
        field_.boundaryFieldRef()[patchID][faceI] = fieldCoeffs_[fieldI][0] * exp( - pow((timeValue - fieldCoeffs_[fieldI][1])/fieldCoeffs_[fieldI][2],2));
      }
    }
    ...
    if (mesh_.objectRegistry::foundObject<volVectorField>(mappingFieldsName_[fieldI])) {
      ...
      forAll(field_.boundaryField()[patchID], faceI) {
        scalar value_ =  fieldCoeffs_[fieldI][0] * exp( - pow((timeValue - fieldCoeffs_[fieldI][1])/fieldCoeffs_[fieldI][2],2));
...
void Foam::gaussianBoundaryMappingModel::write(Ostream& os) const
{
