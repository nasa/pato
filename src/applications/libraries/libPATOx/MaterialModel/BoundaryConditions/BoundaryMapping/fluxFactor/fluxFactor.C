...
fluxMap_(meshLookupOrConstructScalar(mesh, fluxMapName.c_str(), dimensionedScalar("0", dimless, scalar(0.0)))),
pressureMap_(meshLookupOrConstructScalar(mesh, pressureMapName.c_str(), dimensionedScalar("0", dimless, scalar(0.0)))),
...
moreThanThresholdFlag_(dict_.lookupOrDefault<Switch>("moreThanThresholdFlag",true))
{
  ...
  Info << "The fluxFactorMap must have 3 columns: "
  ...
  if (rawTableFracFF != 0) {
    ...
    Info << "The fluxFactorMap has been read (" << rawTableFF << " lines)." << nl;
  }
