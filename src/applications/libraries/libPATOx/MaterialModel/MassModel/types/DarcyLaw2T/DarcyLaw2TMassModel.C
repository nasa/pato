...
Tg_(meshLookupOrConstructScalar(mesh,"Tg")),
...
void Foam::DarcyLaw2TMassModel::beforeSolve()
{
  ...
  Beta.ref()  = eps_g() * M() / (R * Tg_());
  Beta.boundaryFieldRef()  = eps_g.boundaryField() * M.boundaryField() / (R.value() * Tg_.boundaryField());
