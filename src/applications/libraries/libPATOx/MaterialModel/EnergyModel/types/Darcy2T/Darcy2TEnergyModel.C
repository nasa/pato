...
Foam::Darcy2TEnergyModel::Darcy2TEnergyModel
(
    const fvMesh& mesh,
...
Gamma_GHg(createVolField<tensor>("Gamma_GHg",((h_g*p*M)/(mu*R*Tg))*K*p*M/(R*Tg))),
...
void Foam::Darcy2TEnergyModel::update()
{
...
void Foam::Darcy2TEnergyModel::beforeSolve()
{
  ...
  Gamma_GHg.boundaryFieldRef() =
      ...
      *p.boundaryField()*M.boundaryField()
...
void Foam::Darcy2TEnergyModel::afterSolve()
{
  if(this->debug_) {
    forAll(Ts, cellI) {
      if (Ts[cellI]<0) {
