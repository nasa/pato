...
fieldFactors_(simpleMaterialPropertiesModel::materialDict_.subDict("MaterialProperties").lookupOrDefault<List<Tuple2<word,scalar>>>("fieldFactors",List<Tuple2<word,scalar>>(0)))
{
  Info << "fieldFactors = " << fieldFactors_ << endl;
  wordList matPropFieldNames = {"cp_v", "h_v", "ki_v", "kj_v", "kk_v", "emissivity_v", "absorptivity_v", "cp_c", "h_c", "ki_c", "kj_c", "kk_c", "emissivity_c", "absorptivity_c"};
  ...
  forAll(fieldFactors_, i) {
    if (!foundInList(fieldFactors_[i].first(), matPropFieldNames)) {
      FatalError << fieldFactors_[i].first() << " field not found in the material properties fields: " << matPropFieldNames << exit(FatalError);
    }
  }
  ...
  forAll(factorList_char_, i) {
      forAll(fieldFactors_, j) {
          if (fieldFactors_[j].first() == matPropFieldNames[i+factorList_virgin_.size()]) {
  ...
  virginObject.reset(new LinearInterpolation_Factor_MaterialPropertiesObject(readCharredVirginDict("virgin"), factorList_virgin_));
  charObject.reset(new LinearInterpolation_Factor_MaterialPropertiesObject(readCharredVirginDict("char"), factorList_char_));
}
