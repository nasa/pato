...
#include "darcyFlowRatePressureFvPatchScalarField.H"
#include "volFields.H"
...
darcyFlowRatePressureFvPatchScalarField::darcyFlowRatePressureFvPatchScalarField
(
    const fvPatch& p,
...
darcyFlowRatePressureFvPatchScalarField::darcyFlowRatePressureFvPatchScalarField
(
    const darcyFlowRatePressureFvPatchScalarField& psf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
vectorField darcyFlowRatePressureFvPatchScalarField::updateValues
(
    const RhoType& rho
)
{
...
void darcyFlowRatePressureFvPatchScalarField::updateCoeffs()
{
  if (updated()) {
    return;
  }
  ...
  const Field<scalar>& mu =
      patch().template lookupPatchField<volScalarField, vector>("mu_g");

  const Field<tensor>& K =
      patch().template lookupPatchField<volTensorField, tensor>("K");

  gradient() = - mu*(U_ & nf)/((K & nf) & nf);

  fixedGradientFvPatchScalarField::updateCoeffs();
}


void darcyFlowRatePressureFvPatchScalarField::write(Ostream& os) const
{
...
makePatchTypeField(fvPatchScalarField, darcyFlowRatePressureFvPatchScalarField);
