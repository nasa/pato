...
fluxFactorMapFileName_(dict.lookup("fluxFactorMapFileName"))
{
  ...
  Info << "2D-axi_fluxMap Boundary Mapping:\nfields = ( ";
  ...
  OFstream os(listFiles_[(int) indexes[0]].name()+".fluxMap");
  os << fluxFactor_list[2].fluxMap().boundaryField()[1] << endl;
}
...
void Foam::twoDaxiFluxMapTimeBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)
{
  if(foundInList(fieldName,mappingFieldsName_ )) {
    ...
    OFstream os("fluxFactor.fluxMap");
    forAll(factor_.boundaryField()[patchID], faceI) {
      ...
      if (timeValue==66) {
        os << f_times_[0] <<  " " << f_times_[1] <<  " " << factor_data[0] <<  " " <<  factor_data[1] <<  " " << factor_.boundaryFieldRef()[patchID][faceI] << endl;
      }
    }
