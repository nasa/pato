...
#include "twoDaxiInterpolateNearestBoundaryMappingModel.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::twoDaxiInterpolateNearestBoundaryMappingModel::twoDaxiInterpolateNearestBoundaryMappingModel
(
    const fvMesh& mesh,
...
{
  Info << simpleModel::getTabLevel() << "2D-axi Interpolate Nearest Boundary Mapping:" << endl;
  ...
  if (timesMappingFileData_.size()==0) {
    FatalErrorInFunction <<  "2D-InterpolateNearest method: input files not found using \"mappingFileName\".\nUsage example: mappingFileName \"" << (word) mappingFileName_ << "_10\"; //input file name for time = 10s"<< exit(FatalError);
  }
  if (timesMappingFileData_.size()<2) {
    FatalErrorInFunction <<  "2D-InterpolateNearest method needs at least 2 input files using \"mappingFileName\".\nUsage example: mappingFileName \"" << (word) mappingFileName_ << "_10\"; //input file name for time = 10s"<< exit(FatalError);
  }
  // Sort times and data
  ...
  forAll(listMappingFileData_, fileI) {
    ...
    if (mappingFileData_.size() < 4) {
      FatalErrorInFunction << mappingFileName_ << " column size is less than 4.\n The input file must have at least 4 variables: x,y,z,field1 \"" << exit(FatalError) ;
    }
  }
...
void Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)
{
  if(foundInList(fieldName,mappingFieldsName_ )) {
    ...
    if(debug_) {
      Info << "--- currentTimePatchesDataFields_ ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    currentTimePatchesDataFields_[patchID][fieldI]=timeValue;
    int index_time=-1;

    if(debug_) {
      Info << "--- timesMappingFileData_ ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    ...
    if(debug_) {
      Info << "--- mappingFileDataI_ ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    ...
    if(debug_) {
      Info << "--- times_ ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
      Info << "--- timesMappingFileData_ = " << timesMappingFileData_ << " --- Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
      Info << "--- timeValue = " << timeValue << " --- Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
      Info << "--- index_time = " << index_time << " --- Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    ...
    if(debug_) {
      Info << "--- mesh_.objectRegistry::foundObject<volScalarField>(mappingFieldsName_[fieldI]) ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    ...
    if(debug_) {
      Info << "--- mesh_.objectRegistry::foundObject<volVectorField>(mappingFieldsName_[fieldI]) ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    ...
    if(debug_) {
      Info << "--- mesh_.objectRegistry::foundObject<volTensorField>(mappingFieldsName_[fieldI]) ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
    ...
    if(debug_) {
      Info << "--- end ---  Foam::twoDaxiInterpolateNearestBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)" << endl;
    }
  }
}

Foam::scalar Foam::twoDaxiInterpolateNearestBoundaryMappingModel::interpolateNearest(label patchID, label faceI, label fieldI, List<scalarList>& mappingFileData)
{
  if(debug_) {
...
void Foam::twoDaxiInterpolateNearestBoundaryMappingModel::write(Ostream& os) const
{
