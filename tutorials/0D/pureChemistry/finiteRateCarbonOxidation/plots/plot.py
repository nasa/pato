...
ax.set_ylabel('Reaction rate per specie (g/m3/s)', fontsize=16)
...
plot(dataT[:,1],dataRR[:,1]*1e3,ls='solid', color = "red",linewidth=2, label= "O2", alpha = 0.9)
dataRR=patoPlot.dataList[3]
plot(dataT[:,1],dataRR[:,1]*1e3,ls='dashed', color = "red",linewidth=2, label = "C(gr)", alpha = 0.9)
dataRR=patoPlot.dataList[2]
plot(dataT[:,1],dataRR[:,1]*1e3,ls='dashed', color = "green",linewidth=2, label = "CO", alpha = 0.9)
dataRR=patoPlot.dataList[4]
plot(dataT[:,1],dataRR[:,1]*1e3,ls='solid', color = "green",linewidth=2, label = "CO2", alpha = 0.9)
# tight layout
plt.tight_layout()
# add grid
plt.grid()
