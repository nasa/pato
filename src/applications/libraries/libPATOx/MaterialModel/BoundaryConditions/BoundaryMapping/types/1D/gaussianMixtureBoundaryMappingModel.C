...
Foam::gaussianMixtureBoundaryMappingModel::gaussianMixtureBoundaryMappingModel
(
    const fvMesh& mesh,
...
simpleBoundaryMappingModel(mesh, neededFields, dict)
{
  ...
  Info << "gaussian mixture Boundary Mapping:" << endl;
  fieldCoeffs_.resize(mappingFieldsName_.size());
  wordList coefNames_; // Gaussian coeffient names
  coefNames_.append("a1");
  coefNames_.append("b1");
  coefNames_.append("c1");
  coefNames_.append("a2");
  coefNames_.append("b2");
  coefNames_.append("c2");
  ...
  forAll(mappingFieldsName_, fieldI) {
    ...
    Info  << fieldCoeffs_[fieldI][0] << "*exp[-{(t-" << fieldCoeffs_[fieldI][1] << ")/" << fieldCoeffs_[fieldI][2] << "}^2] + " << fieldCoeffs_[fieldI][3] << "*exp[-{(t-" << fieldCoeffs_[fieldI][4] << ")/" << fieldCoeffs_[fieldI][5] << "}^2]" << endl;

  }

  // verify mappingFieldsColumn == 0 for type 0: e.g. p(t) = a * exp (-(t-b)^2/c)
  forAll(mappingFieldsColumn_, columnI) {
    if(mappingFieldsColumn_[columnI] != 0) {
      FatalErrorInFunction << "Second value of the \"mappingFields\" defines the type of the gaussian. Only type 0 is available."
                           << nl << "Type 0: f(t) = a * exp (-{(t-b)/c}^2)" << exit(FatalError);
    }
  }
...
void Foam::gaussianMixtureBoundaryMappingModel::update(scalar timeValue, label patchID, word fieldName)
{
  if(foundInList(fieldName,mappingFieldsName_ )) {
    ...
    if (mesh_.objectRegistry::foundObject<volScalarField>(mappingFieldsName_[fieldI])) {
      ...
      forAll(field_.boundaryField()[patchID], faceI) {
        field_.boundaryFieldRef()[patchID][faceI] = fieldCoeffs_[fieldI][0] * exp( - pow((timeValue - fieldCoeffs_[fieldI][1])/fieldCoeffs_[fieldI][2],2)) + fieldCoeffs_[fieldI][3] * exp( - pow((timeValue - fieldCoeffs_[fieldI][4])/fieldCoeffs_[fieldI][5],2));
      }
    }
    ...
    if (mesh_.objectRegistry::foundObject<volVectorField>(mappingFieldsName_[fieldI])) {
      ...
      forAll(field_.boundaryField()[patchID], faceI) {
        scalar value_ =  fieldCoeffs_[fieldI][0] * exp( - pow((timeValue - fieldCoeffs_[fieldI][1])/fieldCoeffs_[fieldI][2],2)) + fieldCoeffs_[fieldI][3] * exp( - pow((timeValue - fieldCoeffs_[fieldI][4])/fieldCoeffs_[fieldI][5],2));
...
void Foam::gaussianMixtureBoundaryMappingModel::write(Ostream& os) const
{
  os.writeKeyword("mappingType") << "\"gaussianMixture\"" << token::END_STATEMENT << nl;
