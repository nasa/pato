...
const Foam::word Foam::simpleGasPropertiesModel::modelName="GasProperties";
...
)
{
  ...
  if (MaterialDict.isDict("GasProperties")) {
    ...
    GasPropertiesModelTypeName =
        word(GasPropertiesModelDict.lookupOrDefault<word>("GasPropertiesType","no"));
  ...
  return autoPtr<simpleGasPropertiesModel>(cstrIter()(mesh, regionName));
}
